...
export default function ImportCredentialsDialog({ onImport }: ImportCredentialsDialogProps) {
  ...
  const parseCredentials = (text: string, format: "text" | "csv" = "text"): Credential[] => {
    ...
    let category = 'Imported'; // Default to 'Imported' only if no category is provided
    let startIndex = 1;
    
    // Check if second line looks like a category (not an email, not tab-separated, not containing multiple spaces)
    if (lines.length > 1 && !lines[1].includes('@') && !lines[1].includes('\t') && !(/\s{2,}/).test(lines[1])) {
      category = lines[1]; // Use the second line as category
      startIndex = 2; // Start parsing credentials from the third line
    }
